/* Fanqiang Guide chat v1.0.1. No credential or conversation storage in the browser. */
(() => {
  "use strict";
  ...
  function renderAnswerText(element, value) {
    ...
    while (cursor < text.length) {
      ...
      else fragment.append(node("strong", "", text.slice(start + 2, end)));
      cursor = end + 2;
    }
  ...
  async function readAnswer(response, message, mode) {
    ...
    const parser = new SSEParser((event, raw) => {
      ...
      if (event === "artifact") {
        ...
        renderAnswerText(message.body, "工程书已生成，可以下载后交给你自己的 AI。");
      }
