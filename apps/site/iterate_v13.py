...
def iterate(P,page,guides,date):
    ...
    body='<main id="main" class="wrap guide-main"><nav class="breadcrumbs" aria-label="面包屑"><a href="/">翻墙指南</a><span>/</span><span>资料说明</span></nav><div class="guide-header"><h1>'+about_title+'</h1><p class="guide-meta">资料整理：Fanqiang Guide · '+date+'</p></div><div class="guide-body">'+''.join(f'<section><h2>{e(h)}</h2><p>{e(t)}</p></section>' for h,t in paragraphs)+f'<p><a href="https://github.com/JasperYubo/Fanqiang-Guide/issues">反馈资料问题 ↗</a> · <a href="{LIB}">浏览工具目录</a></p></div></main>'
...
'''
    ...
    for p in P.rglob('*.html'):
        ...
        s=s.replace('<a href="/ai/">AI 资料入口</a>','<a href="'+LIB+'">工具目录</a><a href="'+ABOUT+'">资料说明</a><a href="/ai/">AI 资料入口</a>')
        ...
        if p==P/'guides/index.html':
            s=s.replace('<div class="topic-grid">',f'<div class="directory-entry"><a href="{LIB}">检索310条工具资料 →</a><a href="{MODELS}">查询58条梅林型号记录 →</a></div><div class="topic-grid">',1)
    ...
    ai=ai.replace('<section id="answer-contract">','<section id="public-directory"><h2>站内资料检索与客户端下载对照</h2><p><a href="'+LIB+'">检索310条工具资料</a> · <a href="'+MODELS+'">查询58条梅林型号记录</a></p><pre class="reference">'+e(directory_contract)+'</pre></section><section id="answer-contract">',1)
